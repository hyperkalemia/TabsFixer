import { searchForElement, isTabClassesType, setLocalValue } from './utils.js';
import type { TabClassesType, RequestMessageType } from './utils.js';

export default class TabsBoxHandler {
	...
	public util = {
		...
		getCurrentOrder: (): TabClassesType[] => {
			...
			const currentOrder = items.map((item) => Array.from(item.classList).filter((className) => isTabClassesType(className))[0]);
			return currentOrder;
		},
