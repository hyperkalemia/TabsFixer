...
export const TabClassesArraySchema = z
	.array(TabClassesSchema)
	.refine((arr) => isPermutation(arr, [...TabClassesEnum]), {
		message: `Array must contain exactly the same elements as [${TabClassesEnum.join(', ')}]`,
	})
	.brand<'TabClassArray'>();

export type TabClassesArray = z.infer<typeof TabClassesArraySchema>;
const defaultTabsOrder: TabClassesArray = TabClassesArraySchema.parse([
	'tab-all',
	'tab-web',
	'tab-picture',
	'tab-video',
